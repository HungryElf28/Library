...
namespace Library.Application.Services
{
    public class BookmarkService
    {
        ...
        public async Task<Bookmark> AddAsync(int userId, int bookId, int page, string? note)
        {
            var bookmark = new Bookmark(0, userId, bookId, page, note, DateTime.UtcNow);
            return await _repo.AddAsync(bookmark);
        }

        public async Task UpdateAsync(int id, string? note)
        {
            var bookmark = new Bookmark(id, 0, 0, 0, note, DateTime.UtcNow);
